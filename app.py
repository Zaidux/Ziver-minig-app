...
This Flask application provides endpoints for user registration and login. It uses PostgreSQL as the database and bcrypt for password hashing.
...
from flask import Flask, request, jsonify import psycopg2 import bcrypt

Initialize Flask app
app = Flask(name)

Database connection configuration
DB_HOST = "127.0.0.1" DB_NAME = "ziver_db" DB_USER = "termux_user" DB_PASSWORD = "" # Add your password if set
...
@app.route("/", methods=["GET"]) def home(): """ Health check endpoint.

Returns:
    JSON: A welcome message.
"""
return jsonify({"message": "Welcome to Ziver Mining App Backend!"})
@app.route("/register", methods=["POST"]) def register(): """ Registration endpoint for new users.
...
try:
    ...
    if conn:
        cursor.close()
        conn.close()
@app.route("/login", methods=["POST"]) def login(): """ Login endpoint for existing users.
